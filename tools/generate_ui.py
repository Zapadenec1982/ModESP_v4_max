...
class UIJsonGenerator:
                    ...
                    "widgets": [
                        {"key": "_action.grid", "widget": "actions_grid",
                         "actions": [
                             ...
                             {"id": "reset_settings", "label": "Скинути налаштування",
                              "icon": "\u21ba", "api_endpoint": "/api/factory-reset",
                              "confirm": "Всі налаштування будуть скинуті до заводських значень. WiFi та підключення до хмари збережуться. Продовжити?",
                              "style": "warning"},
                             {"id": "factory_reset_full", "label": "Повне скидання",
                              "icon": "\u26a0", "api_endpoint": "/api/factory-reset-full",
                              "confirm": "ВСІ дані будуть стерті, включаючи WiFi, MQTT та логін. Пристрій потребуватиме повторного налаштування. Продовжити?",
                              "style": "danger"},
                         ]},
                    ],
                },
    ...
    def _bindings_page(self, bindings, board, driver_manifests,
        ...
        for req in (equip_requires or []):
            ...
            role_name = req["role"]
            # Explicit zone_min from manifest overrides auto-detection
            if "zone_min" in req:
                zone_min = req["zone_min"]
            else:
...
class StateKeysGenerator:
    ...
    def generate(self, manifests, project=None):
        ...
        lines = [
            "#pragma once",
            "// Auto-generated by generate_ui.py — DO NOT EDIT",
            "// Enum + short names for namespace resolver (Block A)",
            "",
            "#include <cstdint>",
            "#include <cstddef>",
            "",
            "namespace modesp::keys {",
            "",
        ]
        ...
        for m in manifests:
            ...
            # Extract short names: "thermostat.setpoint" → "setpoint"
            prefix = module_name + "."
            short_keys = []
        ...
        lines.append("// Used by InputBinding as fallback keys")
        for m in manifests:
            module_name = m.get("module", "unknown")
            inputs = m.get("inputs", {})
            if not inputs:
                continue
            lines.append(f"// {module_name} inputs:")
            for input_key, info in inputs.items():
                const_name = input_key.upper().replace(".", "_")
                lines.append(f'inline constexpr const char* INPUT_{const_name} = "{input_key}";')
            lines.append("")

        lines.append("}  // namespace modesp::keys")
        lines.append("")
        return "\n".join(lines)


class ModbusRegmapGenerator:
    """Generates generated/modbus_regmap.h — Modbus register map from manifests.

    Address allocation strategy (stable — adding keys doesn't shift existing):
      Input Registers (RO, 30xxx):  module gets a fixed block of 50 regs
      Holding Registers (RW, 40xxx): module gets a fixed block of 50 regs
    ...
    def generate(self, manifests, project=None):
        ...
        for m in manifests:
            mod = m.get("module", "?")
            input_base = self.MODULE_INPUT_BASE.get(mod)
            holding_base = self.MODULE_HOLDING_BASE.get(mod)
            input_idx = 0
            holding_idx = 0
            ...
            for ns, orig_mod, key, info in expanded:
                ...
                if stype == "float":
                    ...
                    continue  # skip string keys (not representable in uint16)

                if access == "readwrite":
                    if holding_base is None:
                        continue
                    addr = holding_base + holding_idx
                    holding_idx += 1
                    holding_entries.append((key, addr, scale, reg_type))
                else:
                    if input_base is None:
                        continue
                    addr = input_base + input_idx
                    input_idx += 1
                    input_entries.append((key, addr, scale, reg_type))

            # Check overflow
            if input_idx > 49:
                errors.append(f"[modbus] {mod}: {input_idx} input regs exceeds 49-slot block")
            if holding_idx > 49:
                errors.append(f"[modbus] {mod}: {holding_idx} holding regs exceeds 49-slot block")
        ...
        lines = [
            "#pragma once",
            "// Auto-generated by generate_ui.py — DO NOT EDIT",
            "// Modbus register map for ModESP PRO",
            "",
            "#include <cstdint>",
            "#include <cstddef>",
            "",
            ...
            "    int16_t scale;           // multiplier (10 for float x10, 1 for int/bool)",
        ...
        lines.append("// Input Registers (FC04, read-only sensor/status values)")
        lines.append("static constexpr ModbusRegEntry MODBUS_INPUT_REGS[] = {")
        if input_entries:
            for key, addr, scale, rtype in input_entries:
                lines.append(f'    {{"{key}", {addr}, {scale}, {rtype}}},')
        else:
            lines.append('    {"", 0, 0, 0},  // placeholder')
        lines.append("};")
        lines.append(f"static constexpr size_t MODBUS_INPUT_REG_COUNT = {len(input_entries)};")
        lines.append("")

        # Holding registers (read-write)
        lines.append("// Holding Registers (FC03/FC06/FC16, read-write setpoints/params)")
        lines.append("static constexpr ModbusRegEntry MODBUS_HOLDING_REGS[] = {")
        if holding_entries:
            for key, addr, scale, rtype in holding_entries:
                lines.append(f'    {{"{key}", {addr}, {scale}, {rtype}}},')
        else:
            lines.append('    {"", 0, 0, 0},  // placeholder')
        lines.append("};")
        lines.append(f"static constexpr size_t MODBUS_HOLDING_REG_COUNT = {len(holding_entries)};")
        lines.append("")
...
def main():
    ...
    with open(modbus_path, "w", encoding="utf-8") as f:
        f.write(modbus_h)
    # Count registers
    input_count = modbus_h.count('"equipment.') + modbus_h.count('"thermostat.') + \
                  modbus_h.count('"defrost.') + modbus_h.count('"protection.') + \
                  modbus_h.count('"lighting.') + modbus_h.count('"datalogger.')
    print(f"  + {modbus_path} (Modbus register map)")
    files_written += 1
    ...
    with open(keys_path, "w", encoding="utf-8") as f:
        f.write(keys_h)
    # Count modules with keys
    key_modules = sum(1 for m in manifests if m.get("state"))
    key_total = sum(len(m.get("state", {})) for m in manifests)
    print(f"  + {keys_path} ({key_modules} modules, {key_total} keys)")
    files_written += 1

    # 7. features_config.h (only if resolver available)
    if resolver:
